        ...
        .then((response) => {
            for(let i =0; i < response.data.length; i++) {
              const newParagraph = document.createElement('p')
                newParagraph.textContent = 'oh My'
                ohMy.appendChild(newParagraph)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/Test-Param')
        .then((response) => {
            console.log(response.data)
    })

    const repeatText = document.getElementById('repeat-text')
    repeatText.textContent = 'repeat my param'
...
// CODE HERE
const repeatMyQuery = () => {
axios.get('http://localhost:3000/repeat?myquery=figuring-out-JavaScript')
        .then((data) => {
            console.log(data)
    })

}    
    document.querySelector('#query-button').addEventListener('click', repeatMyQuery)
